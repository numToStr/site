...
<Callout>
    For this particular setup, I am using 2x2 matrix with 1 encoder on each half
    and using `info.json` for most of the configuration.
</Callout>
...
```json filename="info.json" {5}
{
    "encoder": {
        "enabled": true,
        "rotary": [{ "pin_a": "GP10", "pin_b": "GP11", "resolution": 2 }]
    }
}
```
...
```jsonc filename="info.json"
{
    "split": {
        ...
        "encoder": {
            "enabled": true,
            "rotary": [{ "pin_a": "GP11", "pin_b": "GP10", "resolution": 4 }],
        },
    },
}
```
...
<Callout>
    `qmk` cli is the only(?) way to flash the firmware, QMK toolbox is useless
    for RP2040
</Callout>
